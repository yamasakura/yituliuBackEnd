...
public interface StageResultSetInfoService {
    ...
    //添加T3类材料的理智转化率
    List<List<StageResultData>> setStageResultPercentageT3(Integer times, Double efficiency);
    //添加T2类材料的理智转化率
    List<List<StageResultData>> setStageResultPercentageT2(Integer times, Double expect);
    //添加已关闭活动副本的理智转化率
    List<List<StageResultData>> setClosedActivityStagePercentage(String[] actNameList);
